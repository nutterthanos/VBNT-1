...
local function decode_result(s)
  -- json.decode does not ensure the passed in value is actually a complete
  -- JSON object. But we require the result to be a complete JSON object.
  if s and s:match("%s*{") then
    local decoded, last = json.decode(s)
    if s:match("%s*", last) then
      -- the string is completely decoded. Any leftover chars are whitespace
      return decoded
    end
  end
end
...
function Package:advance_state(operation_ID, desired_state, return_value)
  ...
  if return_value then
    ...
    -- Try to decode it.
    decoded_value = decode_result(return_value)
